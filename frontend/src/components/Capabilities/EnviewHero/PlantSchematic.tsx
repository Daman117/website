...
const Tower: React.FC<{
  ...
  d?: number;
}> = ({ gx, gy, h, r = 0.34, accent = '#077e9b', d = 0 }) => {
...
const Cooler: React.FC<{ gx: number; gy: number; w?: number; d?: number; z?: number; ph?: number }> = ({
  ...
  return (
    <g className="evh-unit" style={{ '--d': `${ph}s` } as React.CSSProperties}>
      {/* legs */}
      <g stroke="#4d80ef" strokeOpacity="0.6" strokeWidth="0.7">
        <path d={`M${pt(e)}v${z}`} />
        <path d={`M${pt(c)}v${z}`} />
      </g>
      <path
      ...
      />
      {/* fin tubes */}
      <g stroke="#066f89" strokeOpacity="0.22" strokeWidth="0.4">
      ...
      {/* fan rings */}
      <g fill="none" stroke="#088aaa" strokeOpacity="0.45" strokeWidth="0.5">
...
const Sphere: React.FC<{ gx: number; gy: number; r?: number; d?: number }> = ({
  ...
  return (
    <g className="evh-unit" style={{ '--d': `${d}s` } as React.CSSProperties}>
      <g stroke="#4d80ef" strokeOpacity="0.65" strokeWidth="0.7">
        {[-0.8, -0.3, 0.3, 0.8].map((f) => (
          <path key={f} d={`M${(base.x + f * r).toFixed(1)} ${(cy + r * 0.7).toFixed(1)}L${(base.x + f * r * 1.15).toFixed(1)} ${base.y.toFixed(1)}`} />
        ))}
      </g>
      <circle cx={base.x.toFixed(1)} cy={cy.toFixed(1)} r={r} fill="#f4f6fb" stroke="#134ecf" strokeOpacity="0.55" strokeWidth="0.7" />
      {/* meridian + equator, so it reads as a sphere and not a disc */}
      <g fill="none" stroke="#066f89" strokeOpacity="0.24" strokeWidth="0.5">
      ...
      <path d={`M${(base.x - r * 0.62).toFixed(1)} ${(cy - r * 0.55).toFixed(1)}a${(r * 0.7).toFixed(1)} ${(r * 0.7).toFixed(1)} 0 0 1 ${(r * 0.5).toFixed(1)} -${(r * 0.3).toFixed(1)}`} fill="none" stroke="#0f3ea5" strokeOpacity="0.35" strokeWidth="0.7" />
    </g>
  );
};
...
const Flare: React.FC<{ gx: number; gy: number; h?: number; d?: number }> = ({ gx, gy, h = 96, d = 0 }) => {
  ...
  return (
    <g className="evh-unit" style={{ '--d': `${d}s` } as React.CSSProperties}>
      {/* derrick */}
      <g stroke="#4d80ef" strokeOpacity="0.7" strokeWidth="0.6" fill="none">
      ...
      <path d={`M${base.x.toFixed(1)} ${base.y.toFixed(1)}V${top.toFixed(1)}`} stroke="#077e9b" strokeOpacity="0.5" strokeWidth="0.9" fill="none" />
      {/* pilot flame */}
      <path
        className="evh-tip"
...
const SceneTrain: React.FC = () => (
  <g>
    ...
    <g opacity="0.75">
      <Line from={[0.4, 0.6]} to={[4.6, 0.2]} color="#088aaa" />
      <Line from={[4.6, 0.2]} to={[5, 4]} color="#2a62dc" />
      <Line from={[0.4, 4.6]} to={[5, 4.6]} color="#088aaa" />
    </g>
    <g>
      <Line from={[0.4, 0.6]} to={[4.6, 0.2]} color="#066f89" flow d={0} />
      <Line from={[4.6, 0.2]} to={[5, 4]} color="#2156cb" flow d={-2.1} />
      <Line from={[0.4, 4.6]} to={[5, 4.6]} color="#066f89" flow d={-4.4} />
      <Line from={[0.6, 1]} to={[0.6, 4.4]} color="#2156c9" flow d={-6.2} />
    </g>

    <Tower gx={3.6} gy={-0.6} h={76} r={0.3} accent="#077e9b" d={-1.4} />
    <Tower gx={4.5} gy={0.1} h={92} r={0.36} accent="#066f89" d={-5.8} />
    <Tower gx={5.4} gy={-0.2} h={68} r={0.28} accent="#077e9b" d={-9.2} />
    <Drum gx={-0.5} gy={0.2} len={2} rad={8} />
    <Tower gx={3.4} gy={3.2} h={40} r={0.26} accent="#088aaa" d={-3.1} />
    <Tower gx={4.3} gy={3.9} h={32} r={0.24} accent="#088aaa" d={-7.5} />
...
const SceneTankFarm: React.FC = () => (
  <g>
    ...
    <g opacity="0.7">
      <Line from={[-0.9, 4.4]} to={[5.2, 4.4]} color="#088aaa" />
    </g>
    <g>
      <Line from={[-0.9, 4.4]} to={[5.2, 4.4]} color="#066f89" flow d={0} />
      <Line from={[0, 4.4]} to={[0, 0.4]} color="#066f89" flow d={-1.6} />
      <Line from={[2, 4.4]} to={[2, 0.4]} color="#2156c9" flow d={-3.4} />
      <Line from={[4, 4.4]} to={[4, 0.4]} color="#066f89" flow d={-5.1} />
    </g>
...
const SceneUtilities: React.FC = () => (
  <g>
    ...
    <g opacity="0.7">
      <Line from={[-0.6, 0.4]} to={[4.8, 0.4]} color="#2a62dc" z={6} />
      <Line from={[-0.6, 3.6]} to={[4.8, 3.6]} color="#088aaa" z={6} />
    </g>
    <g>
      <Line from={[-0.6, 0.4]} to={[4.8, 0.4]} color="#2156cb" z={6} flow d={0} />
      <Line from={[-0.6, 3.6]} to={[4.8, 3.6]} color="#066f89" z={6} flow d={-2.7} />
      <Line from={[4.8, 0.4]} to={[4.8, 3.6]} color="#2156c9" z={6} flow d={-5.3} />
    </g>
    ...
    <Tower gx={4.6} gy={0.6} h={44} r={0.22} accent="#088aaa" d={-3.9} />
...
const SceneReactor: React.FC = () => (
  <g>
    ...
    <g opacity="0.72">
      <Line from={[0.2, 1.2]} to={[4.4, 0.6]} color="#2a62dc" />
      <Line from={[4.4, 0.6]} to={[4.4, 3.4]} color="#088aaa" />
    </g>
    <g>
      <Line from={[0.2, 1.2]} to={[4.4, 0.6]} color="#2156cb" flow d={0} />
      <Line from={[4.4, 0.6]} to={[4.4, 3.4]} color="#066f89" flow d={-3.2} />
      <Line from={[0.2, 3.6]} to={[3.6, 3.6]} color="#2156c9" flow d={-6.1} />
    </g>

    <Flare gx={5.2} gy={-0.8} h={104} d={-2.6} />
    <Tower gx={3.9} gy={0.4} h={58} r={0.3} accent="#066f89" d={-7.2} />
